    ...
    <form method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menghapus data yang dipilih? Data yang dihapus tidak bisa dikembalikan!')">
        <div class="card border-0 shadow-sm rounded-xl overflow-hidden bg-white">
            ...
            <div class="border-b border-gray-200 py-4 px-4 md:px-5 flex flex-col md:flex-row justify-between items-start md:items-center bg-white gap-4">
                <div>
                    <h3 class="text-xl font-bold text-rose-600 m-0"><i class="fas fa-trash-alt mr-2"></i>Hapus Rencana Upah</h3>
                    <p class="text-xs text-gray-500 mt-1">Pilih satu atau beberapa data untuk dihapus sekaligus</p>
                </div>
                ...
                <div class="table-responsive px-3 md:px-4 py-4">
                    <table class="w-full text-left border-collapse table-modern" id="dataTables-hapus">
                        <thead class="bg-gray-50 border-b border-gray-300">
                            <tr>
                                <th class="py-3 px-2 text-center w-12">
                                    <input type="checkbox" id="select_all" class="w-4 h-4 text-blue-600 rounded focus:ring-blue-500 cursor-pointer">
                                </th>
                        ...
                        <tbody class="divide-y divide-gray-100">
                            ...
                            ?>
                                    <tr class="hover:bg-rose-50 transition-colors cursor-pointer" onclick="toggleCheckbox(<?= $data['id_rkk'] ?>)">
                                        <td class="py-3 px-2 text-center" onclick="event.stopPropagation()">
                                            <input type="checkbox" name="id_rkk_pilih[]" value="<?= $data['id_rkk'] ?>" id="check_<?= $data['id_rkk'] ?>" class="check_item w-4 h-4 text-rose-600 rounded focus:ring-rose-500 cursor-pointer">
                                        </td>
                                        <td class="py-3 px-2 text-center text-[15px]"><?= $no++ ?></td>
                                        <td class="py-3 px-2 text-[15px] font-bold text-gray-900"><?= date('d/m/Y', strtotime($data['tgl_rkk'])) ?></td>
                                ...
                                <tr>
                                    <td colspan="5" class="py-10 text-center">
                                        <div class="flex flex-col items-center justify-center text-gray-400">
                                            ...
                                            <p class="text-sm">Belum ada rencana upah yang dapat dihapus.</p>
                                        </div>
                                    </td>
                                </tr>
                            <?php endif; ?>
...
<script>
    ...
    function konfirmasiHapus() {
        ...
        Swal.fire({
            ...
            text: "Data " + checked + " item yang dipilih akan dihapus selamanya!",
            icon: 'warning',
    ...
    // 3. Toggle checkbox saat baris diklik
