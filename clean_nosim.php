...
foreach ($lines as $line) {
    ...
    if (substr($trimmed, 0, 1) == '(' && substr($trimmed, -1) == ')') {
        ...
        // no_sim is index 11 (after removing npwp/bpjs, it's now the 12th value, index 11)
        if (count($values) > 23) { unset($values[11]); $new_lines[] = "  (" . implode(",", array_values($values)) . ")" . $suffix; }
...
echo "no_sim data cleaned from db_hr.sql\n";
?>
